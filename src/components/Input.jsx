...
const Input = ({ setList, list }) => {
  ...
  const handleClick = () => {
    ...
    const listTodoes = list.map((x) => x.todo);
    if (!listTodoes.includes(val)) {
      setList((prev) => [
        ...prev,
        { todo: val, hasPriority: false, done: false, deadline: date },
      ]);
  ...
  return (
    <div className="bg-[#1e2030] w-1/2 mx-auto h-13 rounded-2xl flex align-middle">
      ...
      <a
        href="#"
        onClick={handleClick}
        className="cursor-pointer w-20 h-full rounded-r-2xl text-3xl text-center flex flex-col justify-center hover:outline-[#363a4f] hover:outline-1">
        📝
      </a>
    </div>
  );
